...
const Model = ({bottomSheetLoder, atmRef}) => {
  const {assetsAtmsModel} = useSelector(state => state.AssetsAtmModel);
  ...
  const searchFilterFunction = text => {
    setSearch(text);
    dispatch({
      type: ATMS_ALL_MODEL_SEARCH,
      data: text,
    });
  };
  const searchDATA = text => {
    dispatch({
      type: ALL_ATMS_FILTER_BY_MODEL,
      data: text,
    });
